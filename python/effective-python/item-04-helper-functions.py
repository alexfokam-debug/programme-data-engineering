...
# ==========================================
row = {"user_id": 12345, "amount": "$1,234.56", "date": "2024-06-01", "discount": "10%"}
# ==========================================
# 02 - Version initiale
# ==========================================

final_amount = float(row["amount"].replace("$", "").replace(",", "")) * (
    1 - float(row["discount"].replace("%", "")) / 100
)
...
print(
    calculate_final_amount(row["amount"], row["discount"])
)  # Affiche le montant final calculé avec la fonction auxiliaire
